...
nowcast_date = as.Date("2022-01-02"); window_length = 120
...
# Need to align train feat with test feat; do some LOCF 
target_dates = finalized_data |>
  distinct(time_value) |>
  pull(time_value)

initial_data = map(target_dates, \(tgt_d) epix_as_of(nchs_fullarchive, tgt_d) |> mutate(version = tgt_d)) |>
  list_rbind() |>
  # For each version, take the most recent provisional value as feat 
  group_by(geo_value, version) |>
  slice_max(time_value) |>
  select(-time_value, time_value=version, initial_val=mortality)
...
#| echo: true
#| 
nowcaster = function(x, g, t, wl=120, appx=49) {
  

  finalized_data = epix_as_of(x, t) |>
  ...
  initial_data = map(target_dates, \(tgt_d) epix_as_of(x, tgt_d) |> mutate(version = tgt_d)) |>
    list_rbind() |>
    # For each version, take the most recent provisional value as feat 
    group_by(geo_value, version) |>
    slice_max(time_value) |>
...
first_avail = nchs_fullarchive$DT |>
  filter(time_value >= as.Date("2022-01-01")) |>
  group_by(geo_value, time_value) |>
  filter(version == min(version))

finalized_val = nchs_fullarchive$DT |>
  filter(time_value >= as.Date("2022-01-01")) |>
  group_by(geo_value, time_value) |>
...
#| echo: false
ggplot() + 
  geom_line(data = finalized_val, aes(x = time_value, 
    y = mortality, color = "Finalized")) +
  geom_point(data = finalized_val, aes(x = time_value, 
    y = mortality, color = "Finalized"), shape = 16, size = 1) +
  geom_line(data = first_avail, aes(x = time_value, 
    y = mortality, color = "First available issue")) +
  geom_point(data = first_avail, aes(x = time_value, 
    y = mortality, color = "First available issue"), shape = 16, size=1) +
  geom_line(data = nowcasts, aes(x = target_date, y = nowcast, color = "Nowcast")) +
  geom_point(data = nowcasts, aes(x = target_date, y = nowcast, color = "Nowcast"), 
    shape = 16, size=1) +
  ylab("Mortality") +
  xlab("Date") +
  scale_color_manual(values = c("Nowcast" = "blue", "First available issue" = "red", "Finalized" = "black")) + 
...
mae_value <- mae_data |> 
  group_by(geo_value) |>
  summarise(`Nowcast MAE` = mean(nc_abs_error, na.rm=TRUE),
            `Realtime MAE` = mean(rt_abs_error, na.rm=TRUE))
...
## Sliding average as a smoother

```{r avg-smooth-nowcast}
nowcasts = nowcasts |>
  select(-version) |>
  rename(time_value = target_date) |>
  as_epi_df()

# `window.size` is speicfic to `time_type` of `epi_df`
attr(nowcasts, "metadata")$time_type

nowcasts = epi_slide_mean(.x = nowcasts, .col_names = nowcast, na.rm=TRUE,
  .window_size = as.difftime(tim=4, units="weeks")) |>
  rename(avged_nowcast = slide_value_nowcast)
...
ggplot() + 
  ...
  geom_point(data = first_avail, aes(x = time_value, 
    y = mortality, color = "First available issue"), shape = 16, size=1) +
  geom_line(data = nowcasts, aes(x = time_value, y = avged_nowcast, color = "Smoothed nowcast")) +
  geom_point(data = nowcasts, aes(x = time_value, y = avged_nowcast, color = "Smoothed nowcast"), 
    shape = 16, size=1) +
  ylab("Mortality") +
  xlab("Date") +
  scale_color_manual(values = c("Smoothed nowcast" = "blue", "First available issue" = "red", "Finalized" = "black")) + 
  theme(legend.position = "bottom")
```

```{r}
mae_smoothed = nowcasts |>
  inner_join(finalized_val, by = c("geo_value", "time_value")) |>
  summarise(`Smoothed MAE` =  mean(abs(avged_nowcast - mortality), na.rm=TRUE))
```
...
ggplot(mae_data) +
  ...
  geom_point(aes(x = time_value, y = nc_abs_error), color = "blue", size = 1) +
  geom_line(aes(x = time_value, y = nc_abs_error), color = "blue") +
# +
#   # Horizontal line for Nowcast Mean Absolute Error
#   geom_hline(aes(yintercept = mean(nc_abs_error), color = "Nowcast AE"), linetype = "dashed") +
  
  # Real-Time Absolute Error 
  geom_point(aes(x = time_value, y = rt_abs_error), color = "red", size = 1) +
  geom_line(aes(x = time_value, y = rt_abs_error), color = "red") +
  # 
  # +
  # # Horizontal line for Real-Time Mean Absolute Error
  # geom_hline(aes(yintercept = mean(rt_abs_error), color = "Real-Time AE"), linetype = "dashed") +
  # 
  ...
  scale_color_manual(values = c("Real-Time MAE" = "red", "Nowcast AE" = "blue")) +
...
ggplot() + 
  ...
  geom_ribbon(data = pred_interval, 
              aes(x = target_date, ymin = lower, ymax = upper), 
              alpha = 0.2, fill = "blue") +
  ylab("Mortality") +
  xlab("Date") +
  theme(legend.position = "bottom") +
  scale_color_manual(values = c("Nowcast" = "blue", "Finalized" = "black"))
```
