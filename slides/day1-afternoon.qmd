...
* The ratio model predicts the finalized value of $Y_t$ from $Y_{s}$, the last value included in the report at time $t$.
...
```
The first value here is a version of $Y_{2020-11-30}$ as it was reported in version 2020-12-06.  We expect it to have similar characteristics as $Y_{t - 7\text{ days}}$ as reported in version $t$ for other values of $t$.

## Get our predictor and target data

```{r regression-trial-nowcast-predictor-target}
#| echo: true
...
- And get an error --- some versions $t$ don't include a value $Y_{t-7}$ or $Y_{t-14}$.
  - So let's try including even more lags, but add a limit to make sure we don't include too many.
