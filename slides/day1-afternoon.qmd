...
ggplot(edfg, aes(x = time_value, y = gr_cases)) +
  ...
  scale_color_manual(values = c(3, 6), name = "") +
...
ma_covid_19_raw_cache_dir <- "ma_covid_19_raw_cache_dir"
dir.create(ma_covid_19_raw_cache_dir)

# Function to download and process each Excel file for a given date
process_covid_data <- function(Date, cache_dir = ma_covid_19_raw_cache_dir) {
  ...
  # Define the destination file path for the response
  response_file_path <- file.path(cache_dir, paste0("covid-19-raw-data-", Date, "-response.rds"))

  if (file.exists(response_file_path)) {
    response <- readRDS(response_file_path)
  } else {
    response <- GET(url)

    if (status_code(response) %in% c(403, 404)) {
      # Wait a tiny bit to not monopolize server time:
      Sys.sleep(0.1)
    } else {
      # Stop on errors other than 403 ("forbidden"), 404 ("file not found").
      httr::stop_for_status(response)
      # If we actually received data, wait a more substantial time:
      Sys.sleep(5)
    }

    # Cache the response:
    saveRDS(response, response_file_path)
  }

  if (status_code(response) %in% c(403, 404)) {
    return(NULL)
  } else {
    if ("content-disposition" %in% names(headers(response)) &&
          grepl("zip", headers(response)$`content-disposition`)) {
      return(NULL)
    } else {
      # Read the relevant sheet from the Excel file
      xlsx_file_path <- withr::local_tempfile(fileext = ".xlsx")
      writeBin(content(response), xlsx_file_path)
      data <- read_excel(xlsx_file_path, sheet = "CasesByDate (Test Date)")
...
# FIXME TODO update discussion
start_date <- as.Date("2021-01-04")
end_date <- as.Date("2024-11-01")
y <- process_data_for_date_range(start_date, end_date)  # Raw .xlsx data is first released on Jan. 4, 2021
# TODO investigate earlier zip files?
y
```
...
x <- pub_covidcast(
  source = "google-symptoms",
  signals = c(
    # Some symptoms sets intersecting COVID-19 symptoms, ordered by timeline/severity:
    "s03_smoothed_search", "s01_smoothed_search", "s05_smoothed_search", "s04_smoothed_search",
    # To potentially subtract out influenza activity:
    "s06_smoothed_search",
    # To potentially normalize:
    "scontrol_smoothed_search"
    # TODO revise above or use
    # FIXME TODO start and end date range.... earlier signals.....
  ),
  geo_type = "state",
  time_type = "day",
  geo_values = "ma", # Just for MA to keep it simple (& to go with the case data by test date for that state)
  time_values = "*",
  issues = epirange(19700101, end_date) # Use a date far in the past to ensure we include all issues up to `end_date`
  # TODO check rendering of this comment
) |>
  select(geo_value, time_value, version = issue, signal, value) |>
  pivot_wider(id_cols = c(geo_value, time_value, version),
              names_from = signal, values_from = value) |>
  as_epi_archive(compactify = TRUE)
```
...
<!-- LOCF is used to ensure missing versions is handled by assuming no changes. -->
...
lm_mod_pred <- function(data) {
  # Linear model
  model <- lm(case_rate_7d_av ~ s03_smoothed_search, data = data)

  # Make predictions
  predictions = predict(model,
                        newdata = data |>
                          fill(s03_smoothed_search, .direction = "down") |> 
                          filter(time_value == max(time_value)),
                        interval = "prediction", level = 0.9)
...
library(data.table)
get_as_of_lag1 <- function(archive, time_lag) {
  epikeytime_names <- setdiff(key(archive$DT), "version")
  requests <- unique(archive$DT, by = epikeytime_names, cols = character())[
  , version := time_value + ..time_lag
  ]
  setkeyv(requests, c(epikeytime_names, "version"))
  result_dtbl <- archive$DT[
    requests, roll = TRUE, nomatch = NULL, allow.cartesian = TRUE
  ]
  result_dtbl[]
}
get_as_of_lag2 <- function(archive, time_lag) {
  epikeytime_names <- setdiff(key(archive$DT), "version")
  requests <- unique(archive$DT, by = epikeytime_names, cols = character())[
  , version := time_value + ..time_lag
  ]
  ...
  nms <- names(result_dtbl)
  val_nms <- nms[! nms %in% key(archive$DT)]
  new_val_nms <- paste0(val_nms, "_tlag", time_lag, recycle0 = FALSE)
  nms[match(val_nms, nms)] <- new_val_nms
  setnames(result_dtbl, nms)
  result_dtbl[]
}

## google-symptoms also has comparable or greater lag than the original, and the
## original initial values have an okayish 20% initial MAPE?

## invisible(revision_summary(x))

## TODO remove circa here

get_as_of_lag(archive, 1L) |>
  mutate(time_value = version - 1L, version = NULL) |>
  full_join(archive %>% epix_as_of(.$versions_end), by = c("geo_value", "time_value")) |>
  filter(time_value >= start_date) |>
  with(mean(abs(case_rate_7d_av - case_rate_7d_av_tlag1) / case_rate_7d_av * 100, na.rm = TRUE))

get_as_of_lag(archive, 1L) |>
  mutate(time_value = version - 1L, version = NULL) |>
  full_join(archive %>% epix_as_of(.$versions_end), by = c("geo_value", "time_value")) |>
  filter(time_value >= start_date) |>
  with(quantile(case_rate_7d_av/case_rate_7d_av_tlag1, na.rm = TRUE, probs = (0:10)/10))

## invisible(revision_summary(y))
## invisible(revision_summary(y, within_latest = 0.05))

## y |> revision_summary() |> filter(time_value >= start_date) |> ggplot(aes(time_value, as.integer(min_lag))) + geom_line()

y$DT[time_value >= start_date] |>
  as_epi_archive() |>
  revision_summary() |>
  invisible()
...
y$DT[time_value >= start_date] |>
  as_epi_archive() |>
  revision_summary(within_latest = 0.05) |>
  invisible()

x$DT[time_value >= start_date] |>
  as_epi_archive() |>
  revision_summary(within_latest = 0.05) |>
  invisible()

# TODO update section 4 math

get_as_of_lag(archive, 1L) |>
  mutate(time_value = version - 1L, version = NULL) |>
  full_join(archive %>% epix_as_of(.$versions_end), by = c("geo_value", "time_value")) |>
  ggplot() +
  geom_line(aes(time_value, case_rate_7d_av), colour = "black") +
  geom_line(aes(time_value, case_rate_7d_av_tlag1), colour = "blue")

# TODO plot to see if looks like always-down

archive |>
  epix_slide(~ .x) |>
  ggplot(aes(time_value, case_rate_7d_av, colour = version)) +
  geom_line()

# Define the reference time points (to give the training/test split)
## targeted_nowcast_dates <- seq(as.Date("2021-04-01"), as.Date("2024-11-01"), by = "1 month") 
## produce_nowcast_dates <- targeted_nowcast_dates + 1 # + 1 because the case data is 1 day latent. 
## produce_nowcast_dates <- seq(start_date + 90, end_date, by = "1 month") 
produce_nowcast_dates <- seq(as.Date("2022-04-10"), end_date, by = "1 month") 
targeted_nowcast_dates <- produce_nowcast_dates - 1
...
nowcast_res <- archive |>
  group_by(geo_value) |>
  epix_slide(
    .f = ~ lm_mod_pred(.x),  # Pass the function defined above
    .before = 30,   # Training period of 30 days
    .versions = produce_nowcast_dates, # Determines the day where training data goes up to (not inclusive)
    .new_col_name = "res"
  ) |>
  unnest() |>
  # TODO simplify code
