...
1. Applying computation with rolling window 
...
## Features -- sliding computation on a `epi_df`

* Many aggregations on panel data applies some function to a rolling window of data. 

* By sliding the window, we could apply function to different parts of the snapshot. 

* For example, trailing average is often used to smooth out noises in individual data points.
...
As an example, we show how we can use `epi_slide` to compute a trailing 7-day average. 

## Features -- sliding computation on a `epi_df`

* The simplest way to use `epi_slide` is tidy evaluation. 

* For a groupped `epi_df`, `epi_slide()` applies the computation to groups [separately]{.primary}. 

```{r, grouppped-df-to-slide}
#| echo: false
...
custom_function <- function(x, g, t, ...) {
  ...
  result = data.frame(cases_7dav = mean(x$raw_cases, na.rm = TRUE))
...
## Features -- revision analysis helper function
...
snapshots <- map(versions, function(v) {
  res = epix_as_of(archive_cases_dv_subset_all_states, v) |>
    mutate(version = v)} |>
