...
ca %>%
  ...
  geom_line(aes(alpha = ifelse(time_value < t0_date, 1, .4))) + 
...
test %>% 
  mutate(observed = deaths, 
         `predicted (CV)` = c(rep(NA, k-1), pred_all_past)) %>%
...
```{r cv-error}
errors <- rbind(errors, getErrors(test$deaths[-(1:(k-1))], pred_all_past, 
                                  "time series CV"))
round(errors, 3)
```
...
test %>% 
  mutate(observed = deaths, 
         `predicted (CV)` = c(rep(NA, k-1), pred_all_past), 
         `predicted (trailing + CV)` = c(rep(NA, k-1), pred_trailing)) %>%
  ...
  labs(x = "", y = "Deaths per 100k people") +
  theme(legend.position = "right", legend.title = element_blank()) +
  scale_color_manual(values = c(primary, secondary, tertiary)) +
  scale_x_date(date_labels = "%b %Y") +
...
```{r cv-trailing-error}
errors <- rbind(errors, getErrors(test$deaths[-(1:(k-1))], pred_trailing, 
...
test %>% 
  ...
  labs(x = "", y = "Deaths per 100k people") +
  scale_x_date(date_labels = "%b %Y") +
  scale_color_manual(values = c(primary, secondary, tertiary), name = "") +
  scale_y_continuous(expand = expansion(c(0, 0.05)))
```
...
round(X[1:5, 1:5], 3) # look at first few entries of predictor matrix
```
...
#| fig-width: 8
tibble(
  predictor = names(beta_ridge[, min_ridge]),
  ridge = beta_ridge[, min_ridge],
  lasso = beta_lasso[, min_lasso]) |>
  pivot_longer(-predictor, names_to = 'Method') |>
  ggplot(aes(value, predictor, color = Method)) +
  geom_point(alpha = .8) +
  #geom_line() +
  scale_color_manual(values = c(primary, tertiary), name = "") +
  xlab('Estimated coefficient (log scale)') +
  scale_x_continuous(transform = scales::pseudo_log_trans(sigma = .1),
                     breaks = c(-.1, 0, .1, .25, .5, 1)) +
...
test %>% 
  ...
  geom_ribbon(aes(ymin = pmax(lower, 0), ymax = pmax(upper, 0)), fill = tertiary, alpha = .3) +
...
lm_pred_all_past %>%
  ...
  pivot_longer(cols = c(observed, `predicted (CV)`), 
               names_to = 'Deaths') %>%
  ggplot(aes(time_value, value)) +
  geom_ribbon(aes(ymin = pmax(lower, 0), 
                  ymax = pmax(upper, 0)), fill = tertiary, alpha = .3) +
  geom_line(aes(col = Deaths)) + 
  geom_vline(xintercept = t0_date, lty = 2) +
  labs(x = "", y = "Deaths per 100k people") +
  scale_x_date(date_labels = "%b %Y") +
...
test %>% 
  ...
  pivot_longer(cols = c(predicted, observed), names_to = 'Deaths') %>%
  ggplot(aes(time_value, value)) +
  geom_ribbon(aes(ymin = pmax(lower, 0), ymax = pmax(upper, 0)), fill = tertiary, alpha = .4) +
  geom_line(aes(col = Deaths)) + 
  geom_vline(xintercept = t0_date, lty = 2) +
  labs(x = "", y = "Deaths per 100k people") +
  scale_x_date(date_labels = "%b %Y") +
...
pred_all_past %>%
  ...
  geom_line(aes(y = deaths), col = tertiary) + 
  geom_point(aes(y = median), col = primary) +
  geom_linerange(aes(ymin = lower, ymax = upper), col = primary, alpha = .6) +
...
DJM: something wrong here, intervals don't contain the point preds
...
pred_trailing %>%
  ...
  geom_linerange(aes(ymin = lower, ymax = upper), col = secondary, alpha = .6) +
  geom_vline(xintercept = fc_time_values, lty = 2, col = 'gray50') +
  labs(x = "", y = "Deaths per 100k people") +
  scale_x_date(date_labels = "%b %Y") +
  scale_y_continuous(expand = expansion(c(0, 0.05)), limits = c(0, NA))
