...
get_predictor_training_data(nchs_past_archive, "mortality", 7, "mortality_lag7")
...
predictor_descriptions <- nchs_past_latest |>
  ...
  mutate(predictor_name = paste0(varname, "_lag", lag_days)) |>
  drop_na() |>
...
predictors <- predictor_descriptions |>
  ...
  reduce(full_join, by = c("geo_value", "nowcast_date"))

revision_summary(nchs_fullarchive, mortality, within_latest = 0.05)

target <- nchs_past_latest |>
  filter(time_value <= max(time_value) - 90) |>
  rename(nowcast_date = time_value,
         mortality_stable = mortality)

training_test <- full_join(predictors, target, by = c("geo_value", "nowcast_date"))

training <- training_test |> drop_na()
test <- training_test |> filter(nowcast_date == trial_nowcast_date)

# TODO "lag" -> "realtime_lag"?

quantreg::rq(mortality_stable ~ mortality_lag7 + 0, tau = 0.5, training)

quantreg::rq(mortality_stable ~ mortality_lag14 + 0, tau = 0.5, training)

quantreg::rq(mortality_stable ~ mortality_lag21 + 0, tau = 0.5, training)

quantreg::rq(mortality_stable ~ mortality_lag28 + 0, tau = 0.5, training)

quantreg::rq(mortality_stable ~ mortality_lag7 + mortality_lag14 + 0, tau = 0.5, training)

quantreg::rq(mortality_stable ~ mortality_lag7 + mortality_lag14 + mortality_lag21 + mortality_lag28 + 0, tau = 0.5, training)
...
regression_nowcaster <- function(archive) {
  ...
  training <- training_test |>
    drop_na() |>
    # FIXME port above & separate
    slice_max(nowcast_date, n = 90) |>
    identity()
  test <- training_test |> filter(nowcast_date == .env$nowcast_date)
...
regression_nowcasts |>
  ...
  geom_line(aes(nowcast_date, mort14),
            get_predictor_training_data(nchs_fullarchive, "mortality", 14L, "mort14"),
            colour = "orange")


full_join(
  get_predictor_training_data(nchs_fullarchive, "mortality", 7L, "mortality_lag7"),
  nchs_fullarchive |> epix_as_of(nchs_fullarchive$versions_end),
  by = c("geo_value", "nowcast_date" = "time_value")
) |>
  mutate(ratio =  mortality / mortality_lag7) |>
  ggplot(aes(nowcast_date, ratio)) +
  geom_point() +
  geom_line()

full_join(
  get_predictor_training_data(nchs_fullarchive, "mortality", 14L, "mortality_lag14"),
  nchs_fullarchive |> epix_as_of(nchs_fullarchive$versions_end),
  by = c("geo_value", "nowcast_date" = "time_value")
) |>
  mutate(ratio =  mortality / mortality_lag14) |>
  ggplot(aes(nowcast_date, ratio)) +
  geom_point() +
  geom_line()
...
training |>
  ...
  ggplot(aes(nowcast_date, mortality_stable/value, color = name)) +
  geom_point() +
  geom_line()
