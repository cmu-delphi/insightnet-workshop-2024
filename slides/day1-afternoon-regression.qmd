...
ar_nowcasts |>
  ...
  geom_line(aes(time_value, mort21),
            get_predictor_training_data(nchs_fullarchive, "mortality", 21L, "mort21"),
            colour = "brown") +
  geom_line(aes(time_value, mort28),
            get_predictor_training_data(nchs_fullarchive, "mortality", 28L, "mort28"),
            colour = "green")
...
ratio_adj_nowcasts <- nchs_fullarchive |>
  epix_merge(ratio_nowcasts_archive, sync = "locf") |>
...
ratio_adj_nowcasts_with_info <- nchs_fullarchive |>
  epix_merge(ratio_nowcasts_archive, sync = "locf") |>
...
hosp_mort_archive <- epix_merge(hhs_hosp_archive, nchs_fullarchive_daily, sync = "locf")
...
arx_nowcasts <- hosp_mort_archive |>
  epix_slide(~ regression_nowcaster(.x, arx_settings),
             ...
             .all_versions = TRUE) |>
  rename(prediction_arx = prediction)

arx_nowcasts |>
  full_join(locf_nowcasts |> rename(prediction_locf = prediction),
            by = c("geo_value", "target_date"),
            suffix = c("_arx", "_locf")) |>
  full_join(nchs_fullarchive |> epix_as_of(nchs_fullarchive$versions_end),
            by = c("geo_value", "target_date" = "time_value")) |>
  ...
  geom_line(aes(target_date, mortality)) +
  geom_line(aes(target_date, prediction_locf), colour = "red") +
  geom_line(aes(target_date, prediction_arx), colour = "blue")
```
